...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'waflt-theme-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			array(),
			waflt_theme()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'waflt-theme-navigation', 'async', true );
		wp_script_add_data( 'waflt-theme-navigation', 'precache', true );
		wp_localize_script(
			'waflt-theme-navigation',
			'wafltThemeScreenReaderText',
			array(
				'expand'   => __( 'Expand child menu', 'waflt-theme' ),
				'collapse' => __( 'Collapse child menu', 'waflt-theme' ),
			)
		);
	}
