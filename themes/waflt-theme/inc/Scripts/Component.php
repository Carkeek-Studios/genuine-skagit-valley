...
class Component implements Component_Interface {
	...
	protected function get_js_files() : array {
		...
		$js_files = array(
			'waflt-theme-global-js'  => array(
				'file'         => 'global.min.js',
				'dependencies' => array( 'jquery', 'waflt-theme-popover-js' ),
				'in_footer'    => true,
			),
			'waflt-theme-popover-js' => array(
		 ...
		 */
		$js_files = apply_filters( 'waflt_theme_js_files', $js_files );
	...
	public function register_global() {
		...
		foreach ( $js_files as $handle => $data ) {
			...
			$version = waflt_theme()->get_asset_version( $js_dir . $data['file'] );
			$asset   = $js_uri . waflt_theme()->get_asset_path( $data['file'] );
