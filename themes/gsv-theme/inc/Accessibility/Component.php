...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'gsv-theme-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			array(),
			gsv_theme()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'gsv-theme-navigation', 'async', true );
		wp_script_add_data( 'gsv-theme-navigation', 'precache', true );
		wp_localize_script(
			'gsv-theme-navigation',
			'gsvThemeScreenReaderText',
			array(
				'expand'   => __( 'Expand child menu', 'gsv-theme' ),
				'collapse' => __( 'Collapse child menu', 'gsv-theme' ),
			)
		);
	}
