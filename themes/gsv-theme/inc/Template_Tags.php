 ...
 *
 * Its instance can be accessed through `gsv_theme()`. For example, if there is a template tag called `posted_on`, it can
 * be accessed via `gsv_theme()->posted_on()`.
 */
class Template_Tags {
			...
			if ( ! $component instanceof Templating_Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme templating component %1$s does not implement the %2$s interface.', 'gsv-theme' ),
						gettype( $component ),
		...
		if ( ! isset( $this->template_tags[ $method ] ) ) {
			throw new BadMethodCallException(
				sprintf(
					...
					__( 'The template tag %s does not exist.', 'gsv-theme' ),
					'gsv_theme()->' . $method . '()'
				)
			);
		}
		...
		foreach ( $tags as $method_name => $callback ) {
			...
			if ( ! is_array( $callback ) || ! isset( $callback['callback'] ) ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The template tag method %1$s registered by theme component %2$s must either be a callable or an array.', 'gsv-theme' ),
						$method_name,
						get_class( $component )
					)
				);
			...
			if ( isset( $this->template_tags[ $method_name ] ) ) {
				throw new RuntimeException(
					sprintf(
						/* translators: 1: template tag method name, 2: component class name */
						__( 'The template tag method %1$s registered by theme component %2$s conflicts with an already registered template tag of the same name.', 'gsv-theme' ),
						$method_name,
						get_class( $component )
					)
				);
