...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function initialize() {
		add_filter( 'gsv_theme_js_files', array( $this, 'woocommerce_js_file' ) );
	...
	public function woocommerce_js_file( $js_files ) {
		if ( is_shop() || is_product_category() || is_product() || is_cart() ) {
			$js_files['gsv-theme-woocommerce'] = array(
				'file'         => 'woo.min.js',
	...
	public function woocommerce_register_sidebars() {
		register_sidebar(
			array(
				'name'          => esc_html__( 'Shop Page Footer', 'gsv-theme' ),
				'id'            => 'shop-page-footer',
				'description'   => esc_html__( 'Displayed on the shop page', 'gsv-theme' ),
	...
	public function format_grouped_price_range( $price, $from, $to ) {
		...
		$price = sprintf( _x( '%1$s &plus;', 'Price range: from-to', 'gsv-theme' ), is_numeric( $to ) ? wc_price( $to ) : $to );
		return $price;
	}
