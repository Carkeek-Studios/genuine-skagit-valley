...
class Component implements Component_Interface {
	...
	protected function get_js_files() : array {
		...
		$js_files = array(
			'pip-theme-global-js'  => array(
				'file'         => 'global.min.js',
				'dependencies' => array( 'jquery', 'pip-theme-popover-js' ),
				'in_footer'    => true,
			),
			'pip-theme-popover-js' => array(
		 ...
		 */
		$js_files = apply_filters( 'pip_theme_js_files', $js_files );
		$this->js_files = array();
	...
	public function register_global() {
		...
		foreach ( $js_files as $handle => $data ) {
			...
			$version = pip_theme()->get_asset_version( $js_dir . $data['file'] );
			$asset   = $js_uri . pip_theme()->get_asset_path( $data['file'] );
