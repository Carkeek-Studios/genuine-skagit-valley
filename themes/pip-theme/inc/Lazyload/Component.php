...
class Component implements Component_Interface {
	...
	public function action_customize_register_lazyload( WP_Customize_Manager $wp_customize ) {
		$lazyload_choices = array(
			'lazyload'    => __( 'Lazy-load on (default)', 'pip-theme' ),
			'no-lazyload' => __( 'Lazy-load off', 'pip-theme' ),
		);
		...
		$wp_customize->add_control(
			'lazy_load_media',
			array(
				'label'       => __( 'Lazy-load images', 'pip-theme' ),
				'section'     => 'theme_options',
				'type'        => 'radio',
				'description' => __( 'Lazy-loading images means images are loaded only when they are in view. Improves performance, but can result in content jumping around on slower connections.', 'pip-theme' ),
	...
	public function action_enqueue_lazyload_assets() {
		wp_enqueue_script(
			'pip-theme-lazy-load-images',
			get_theme_file_uri( '/assets/js/lazyload.min.js' ),
			array(),
			pip_theme()->get_asset_version( get_theme_file_path( '/assets/js/lazyload.min.js' ) ),
			false
		);
		wp_script_add_data( 'pip-theme-lazy-load-images', 'defer', true );
		wp_script_add_data( 'pip-theme-lazy-load-images', 'precache', true );
	}
