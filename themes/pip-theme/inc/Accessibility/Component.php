...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'pip-theme-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			array(),
			pip_theme()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'pip-theme-navigation', 'async', true );
		wp_script_add_data( 'pip-theme-navigation', 'precache', true );
		wp_localize_script(
			'pip-theme-navigation',
			'pipThemeScreenReaderText',
			array(
				'expand'   => __( 'Expand child menu', 'pip-theme' ),
				'collapse' => __( 'Collapse child menu', 'pip-theme' ),
			)
		);
	}
